...
- Минимум 1GB RAM, 1 CPU (рекомендуется 2GB+ для сборки)
...
### Создание Swap файла (ОБЯЗАТЕЛЬНО для серверов с 1-2GB RAM)

> ⚠️ **Важно!** Если на сервере меньше 4GB RAM, создайте swap файл для предотвращения "Bus error" при сборке.
...
# Сделать swap постоянным (после перезагрузки)
...
# NextAuth.js
NEXTAUTH_URL="http://85.209.134.156:3005"
...
NEXT_PUBLIC_APP_URL="http://85.209.134.156:3005"
...
# Если возникает "Bus error" или "Killed" - используйте ограничение памяти:
NODE_OPTIONS="--max-old-space-size=1024" npm run build

# Или с большим лимитом (если swap создан):
NODE_OPTIONS="--max-old-space-size=2048" npm run build
```

> 💡 **Совет:** Если сборка всё равно падает, убедитесь что swap файл создан (см. раздел 1).
...
```bash
npm run start
# Приложение будет доступно на http://85.209.134.156:3005
```
...
- [ ] Сайт открывается по адресу http://85.209.134.156:3005
- [ ] Админ-панель доступна: http://85.209.134.156:3005/admin/login
...
### "tsx: not found" при db:seed

```bash
# Переустановите зависимости
npm install

# Или установите tsx глобально
sudo npm install -g tsx
```

### "Bus error" или "Killed" при сборке

Это нехватка оперативной памяти. Решение:

```bash
# 1. Проверить swap
free -h

# 2. Если swap = 0, создать его (см. раздел 1)
sudo fallocate -l 2G /swapfile
sudo chmod 600 /swapfile
sudo mkswap /swapfile
sudo swapon /swapfile

# 3. Запустить сборку с лимитом памяти
NODE_OPTIONS="--max-old-space-size=2048" npm run build
```
